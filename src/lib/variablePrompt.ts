...
function findAspectRatio(body: string): VariablePromptParseResult['aspectRatio'] {
  const match = body.match(/(?:图片|画面)比例为\s*[：:]?\s*(1\s*:\s*1|3\s*:\s*4|4\s*:\s*3|9\s*:\s*16|16\s*:\s*9)(?=$|[\s，,。；;])/u)
  if (!match) return undefined
  const ratio = match[1].replace(/\s+/g, '')
  return SUPPORTED_ASPECT_RATIOS.has(ratio)
    ? ratio as VariablePromptParseResult['aspectRatio']
    : undefined
}
...
export function parseVariablePrompt(prompt: string): VariablePromptParseResult {
  ...
  const unusedNames = definitions.filter((definition) => !usedNames.includes(definition.name)).map((definition) => definition.name)
...
function candidateIndexes(
  ...
  return variables.map((variable, variableIndex) => {
    ...
    return hashString(`${seed}:${outputIndex}:${candidateIndex}:${variableIndex}:${variable.name}`) % variable.options.length
  })
}
...
function chooseCombinations(variables: VariablePromptDefinition[], count: number, seed: string) {
  ...
  for (let outputIndex = 0; outputIndex < count; outputIndex++) {
    ...
    for (let candidateIndex = 0; candidateIndex < candidateCount; candidateIndex++) {
      ...
      const nearestRecentDistance = recent.length > 0
        ? Math.min(...recent.map((selection) => hammingDistance(candidate, selection)))
        : variables.length
      const usagePenalty = candidate.reduce((sum, optionIndex, variableIndex) => sum + usage[variableIndex][optionIndex], 0)
      const score = adjacentDistance * 100 + nearestRecentDistance * 20 - usagePenalty * 12 - candidateIndex * 0.001
      if (score > bestScore) {
        best = candidate
        bestScore = score
      }
...
function renderBody(body: string, variables: VariablePromptDefinition[], selection: number[]) {
  const selectedByName = new Map(variables.map((variable, index) => [variable.name, variable.options[selection[index]]]))
  return body.replace(VARIABLE_MARKER, (marker, rawName: string) => {
...
export function renderVariablePromptBatch(prompt: string, count: number, seed = prompt) {
  ...
  return chooseCombinations(parsed.variables, normalizedCount, seed)
    .map((selection) => renderBody(parsed.body, parsed.variables, selection))
}
