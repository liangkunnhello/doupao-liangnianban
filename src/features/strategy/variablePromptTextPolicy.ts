...
const FORBIDDEN_VARIABLE_NAME = /文案|文字|标题|副标题|卖点|价格|价签|配料|参数|信息列表|标签|品牌|logo|二维码|ocr|文字区|文案区/iu
const FORBIDDEN_OPTION_CONTENT = /文案|文字|标题|副标题|卖点|价格|价签|配料|参数|信息列表|标签|品牌|copy_schema|subject_copy_binding|OCR|Logo|二维码|文字区|文案区/iu
const EXCLUDE_TEXT_CLAUSE = '忽略参考图中的所有文字与文案排版，不生成任何文字、字母、数字、Logo、水印、二维码、标题、标签、按钮或仿按钮控件，不预留文案区或文字安全区。'
...
export function applyVariablePromptTextPolicy(prompt: string, excludeText: boolean) {
  ...
  const forbiddenOption = parsed.variables.find((variable) => variable.options.some((option) => FORBIDDEN_OPTION_CONTENT.test(option)))
  if (forbiddenOption) {
