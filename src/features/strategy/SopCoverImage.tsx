...
export default function SopCoverImage({
  ...
  return (
    <span
      ...
      className={`flex shrink-0 items-center justify-center overflow-hidden bg-[hsl(var(--ds-color-surface-subtle))] text-[hsl(var(--ds-color-text-subtle))] ${className}`}
    >
      {src
        ? <img src={src} alt={alt} className="h-full w-full object-cover" />
        : fallbackText
          ? <span aria-hidden="true" className="text-xs font-semibold">{fallbackText}</span>
