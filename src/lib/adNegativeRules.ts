import {
  DEFAULT_AD_NEGATIVE_RULE_PROFILES,
  type AdNegativeRuleProfile,
  type AppSettings,
} from '../types'
  ...
  const custom = Array.isArray(value)
    ? value.flatMap((item): AdNegativeRuleProfile[] => {
      ...
      return [{
        ...
        source: 'custom',
        platform: 'custom',
        version: typeof record.version === 'number' && Number.isFinite(record.version) ? Math.max(1, Math.trunc(record.version)) : 1,
        updatedAt: typeof record.updatedAt === 'number' && Number.isFinite(record.updatedAt) ? record.updatedAt : Date.now(),
      }]
    })
    : []
  return [...DEFAULT_AD_NEGATIVE_RULE_PROFILES.map((rule) => ({ ...rule })), ...custom.filter((rule, index, list) => list.findIndex((item) => item.id === rule.id) === index)]
}

export function getAdNegativeRule(settings: Pick<AppSettings, 'adNegativeRuleProfiles'>, ruleId: string): AdNegativeRuleProfile {
  return settings.adNegativeRuleProfiles.find((rule) => rule.id === ruleId)
