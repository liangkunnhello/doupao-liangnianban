...
describe('strategy library model', () => {
  ...
  it('treats outputs as optional until explicitly enabled', () => {
    const strategy = createStrategyAsset('product', 'type', 'user')
    strategy.workflow.sop = { resolved: true, mode: 'custom', name: '商品场景 SOP', content: '先确认商品卖点，再完成画面构图。' }
  ...
  it('provides global SOP, export and allocation presets', () => {
    expect(new Set(seedStrategyPresets().map((item) => item.type))).toEqual(new Set(['sop', 'export', 'allocation']))
  })
})
