...
function createAssistantActionInput(
  ...
  schema.alternativePrompt = effective.conceptCategories.map((category) => `${category}：该维度的上位概念描述`).join('\n')
  if (context.hasImage) {
...
function normalizeVisualSkillResult(
  ...
  const alternativePrompt = typeof payload.alternativePrompt === 'string'
...
export async function createAssistantSkillDraft(description: string, opts: { settings: AppSettings; profile: ApiProfile; params: TaskParams }): Promise<AssistantSkillDraft> {
  ...
  const wordEntries: WordEntryConfig = {
    ...
    categories: Array.isArray(rawWord?.categories) ? rawWord!.categories.map(String).filter(Boolean).slice(0, 12) : (rawWord?.enabled ? [...DEFAULT_INFORMATION_FLOW_AD_VARIABLE_CATEGORIES] : []),
