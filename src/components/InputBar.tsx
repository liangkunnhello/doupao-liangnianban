...
function getNodeVisibleTextLength(node: Node): number {
  ...
  if (node instanceof HTMLElement && (node.classList.contains('mention-tag') || node.classList.contains('wildcard-var'))) {
...
function setContentEditableSelection(el: HTMLElement, start: number, end: number) {
  ...
  const findBoundary = (targetOffset: number, edge: 'start' | 'end'): Boundary => {
    ...
    const walk = (current: Node): Boundary | null => {
      ...
      if (current instanceof HTMLElement && (current.classList.contains('mention-tag') || current.classList.contains('wildcard-var'))) {
...
export default function InputBar() {
  ...
  const handleAssistantSaveWordEntries = useCallback((groups: AssistantWordEntryGroup[], options: AssistantWordEntryApplyOptions) => {
    const { savedCount } = saveAssistantWordEntryGroups(groups, options)
          ...
          {inputImageFolder && (
            <div className="mb-2 px-1">
              <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-xl px-3 py-2">
                ...
                <span className="text-xs text-blue-700 dark:text-blue-300 truncate flex-1 min-w-0" title={inputImageFolder.path}>
