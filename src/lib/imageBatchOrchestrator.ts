...
function slotCoveredByInFlight(state: GenerationState, slotIndex: number): boolean {
  if (state.slots[slotIndex] && (state.slots[slotIndex].status === 'submitted' || state.slots[slotIndex].status === 'running' || state.slots[slotIndex].status === 'validating')) {
    return true
  }
  return state.remoteRequests.some(
    (req) => isRequestInFlight(req.status) && req.slotIndexes.includes(slotIndex),
  )
}
...
export function applyRemoteRequestSubmitted(
  ...
  const replacementCount = !existing && planned.reason === 'replacement' ? state.replacementCount + 1 : state.replacementCount
...
export function applyRequestFailure(
  ...
  if (failSlotKinds.includes(kind)) {
    const slots = state.slots.map((slot) =>
      request.slotIndexes.includes(slot.index) && slot.status !== 'done'
        ? { ...slot, status: 'failed' as const, error: errorKindToMessage(kind) }
        : slot,
    )
    const remoteRequests = state.remoteRequests.map((req) =>
      req.id === requestId ? { ...req, status: 'failed' as const, error: kind, updatedAt: Date.now() } : req,
    )
    ...
    if (stopKinds.includes(kind)) {
      return { ...next, status: 'error', error: errorKindToMessage(kind) }
    }
    return next
  }
  ...
  if (request.remoteRequestId) {
    const remoteRequests = state.remoteRequests.map((req) =>
      req.id === requestId ? { ...req, status: 'running' as const, error: kind, updatedAt: Date.now() } : req,
    )
...
export function getRecoverableRequests(state: GenerationState): RemoteGenerationRequest[] {
  return state.remoteRequests.filter(
    (req) => Boolean(req.remoteRequestId) && isRequestInFlight(req.status),
  )
}
...
export function classifyGenerationError(err: unknown, options: ClassifyErrorOptions = {}): GenerationErrorKind {
  ...
  if (/network|fetch|econnreset|econnrefused|timeout|timed?\s*out|aborted|断开|连接失败|failed to fetch|socket/i.test(lower)) {
