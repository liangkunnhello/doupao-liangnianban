...
function normalizeActiveSelection(state: Pick<CompositeStoreState, 'categories' | 'activeCategoryId' | 'activePageId'>) {
...
export const useCompositeStore = create<CompositeStoreState>()(
  persist(
    (set) => ({
      ...
      updateCategory: (categoryId, patch) => set((state) => ({
      ...
      deleteCategory: (categoryId) => set((state) => {
        ...
        const fallback = categories.length ? categories : [createBlankCompositeCategory('category-default', '默认合成')]
        return { categories: fallback, ...normalizeActiveSelection({ ...state, categories: fallback }) }
      }),
      addPage: (categoryId) => set((state) => {
      ...
      duplicatePage: (categoryId, pageId) => set((state) => {
        let nextPageId = state.activePageId
        const categories = state.categories.map((category) => {
          if (category.id !== categoryId) return category
      ...
      deletePage: (categoryId, pageId) => set((state) => {
        const categories = state.categories.map((category) => {
          if (category.id !== categoryId) return category
      ...
      updatePage: (categoryId, pageId, patch) => set((state) => ({
        categories: state.categories.map((category) => (
      ...
      updateLayer: (layerId, patch) => set((state) => ({
        categories: mapActivePreset(state.categories, state.activeCategoryId, state.activePageId, (preset) => ({
          ...preset,
          layers: preset.layers.map((layer) => layer.id === layerId ? { ...layer, ...patch } as CompositeLayer : layer),
        })),
      })),
      ...
      addProductSizeRule: (productId) => set((state) => ({
        products: state.products.map((product) => (
          product.id === productId
            ? { ...product, sizeRules: [...product.sizeRules, createCompositeProductSizeRule(makeId('size'), `尺寸 ${product.sizeRules.length + 1}`)] }
            : product
        )),
      })),
      updateProductSizeRule: (productId, ruleId, patch) => set((state) => ({
        products: state.products.map((product) => (
          product.id === productId
            ? { ...product, sizeRules: product.sizeRules.map((rule) => rule.id === ruleId ? { ...rule, ...patch } : rule) }
            : product
        )),
      })),
      deleteProductSizeRule: (productId, ruleId) => set((state) => ({
        products: state.products.map((product) => {
          if (product.id !== productId) return product
          const sizeRules = product.sizeRules.filter((rule) => rule.id !== ruleId)
          return { ...product, sizeRules: sizeRules.length ? sizeRules : [createCompositeProductSizeRule(`${product.id}-main`, '主尺寸输出')] }
        }),
      })),
      addWatermarkPreset: (preset) => set((state) => ({
        watermarkPresets: [...state.watermarkPresets, preset],
      })),
      updateWatermarkPreset: (presetId, patch) => set((state) => ({
        watermarkPresets: state.watermarkPresets.map((preset) => preset.id === presetId ? { ...preset, ...patch } : preset),
      })),
      duplicateWatermarkPreset: (presetId) => set((state) => {
      ...
      deleteWatermarkPreset: (presetId) => set((state) => ({
      ...
      addWatermarkGroup: (group) => set((state) => ({
        watermarkGroups: [...state.watermarkGroups, group],
      })),
      updateWatermarkGroup: (groupId, patch) => set((state) => ({
        watermarkGroups: state.watermarkGroups.map((group) => group.id === groupId ? { ...group, ...patch } : group),
      })),
      deleteWatermarkGroup: (groupId) => set((state) => ({
      ...
      updateOutputPresetRule: (groupId, ruleId, patch) => set((state) => ({
        outputPresetGroups: (state.outputPresetGroups?.length ? state.outputPresetGroups : createDefaultCompositeOutputPresetGroups()).map((group) => (
          group.id === groupId
...
export function getActiveCompositePage(state: Pick<CompositeStoreState, 'categories' | 'activeCategoryId' | 'activePageId'>) {
