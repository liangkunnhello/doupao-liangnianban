...
export function createAgentBatchPreset(input: Omit<AgentBatchStrategyPreset, 'id' | 'version' | 'createdAt' | 'updatedAt'>, now = Date.now()): AgentBatchStrategyPreset {
  return {
    ...input,
...
export function loadAgentBatchPresets(storage: StorageLike | null = getStorage()): AgentBatchStrategyPreset[] {
  ...
  return parsed.filter((item): item is AgentBatchStrategyPreset => Boolean(item) && typeof item === 'object' &&
    (item as AgentBatchStrategyPreset).version === 1 && typeof (item as AgentBatchStrategyPreset).id === 'string' && typeof (item as AgentBatchStrategyPreset).name === 'string')
}
...
export function upsertAgentBatchPreset(presets: AgentBatchStrategyPreset[], preset: AgentBatchStrategyPreset): AgentBatchStrategyPreset[] {
  const index = presets.findIndex((item) => item.id === preset.id)
  if (index < 0) return [...presets, preset]
  return presets.map((item) => item.id === preset.id ? preset : item)
}

export function applyAgentBatchPreset(rows: BatchTaskInput[], preset: AgentBatchStrategyPreset, overwrite = false): BatchTaskInput[] {
  return rows.map((row) => ({
    ...row,
