...
import { mergeBuiltInSopMetaInstructions, migrateSopLibraryExecutionModes, seedSopGroups, seedSopLibrary, seedSopMetaInstructions, sopLibraryId } from '../strategy/sopLibrary'
...
interface RequirementPrototypeState {
  ...
  replaceKnowledgeInsights: (batchId: string, insights: Omit<KnowledgeInsight, 'id' | 'batchId' | 'createdAt'>[]) => void
...
export const useRequirementPrototype = create<RequirementPrototypeState>()(
  persist(
    (set, get) => ({
      ...
      sopLibrary: seedSopLibrary(seedStrategyPresets()),
      ...
      remainingQuota: (userId) => {
        ...
        const used = get().orders
          .filter((order) => order.createdBy === userId && order.createdAt >= startOfToday() && order.status !== 'cancelled')
      ...
      createOrder: (draft) => {
        ...
        const order: RequirementOrder = {
          ...
          estimatedFinishedAt: createdAt + Math.max(20 * 60_000, preview.totalImages * 45_000 / Math.max(1, state.settings.generationConcurrency)),
        }
        const event: RequirementAuditEvent = {
          id: id('audit'),
          actorId: user.id,
      ...
      createStrategyTest: (materialTypeId) => {
        ...
        const draft: RequirementDraft = {
          ...
          channels: [{ channelId: state.catalog.channels[0].id, ratios: [materialType.supportedRatios?.[0] ?? '16:9'] }],
          materialTypeIds: [materialTypeId],
          quantity: 1,
          urgentRequested: false,
        }
        const preview = planRequirementOrder(draft, {
          ...state.catalog,
          materialTypes: state.catalog.materialTypes.map((item) => item.id === materialTypeId ? materialType : item),
      ...
      createStrategyWorkflowTest: (selectedStrategyId, quantity) => {
        ...
        const order: RequirementOrder = {
          ...
          urgentRequested: false,
          urgentApproved: false,
          estimatedFinishedAt: createdAt + Math.max(10 * 60_000, quantity * 45_000 / Math.max(1, state.settings.generationConcurrency)),
          isTest: true,
      ...
      attachTask: (orderId, unitId, taskId) => set((state) => ({
        orders: state.orders.map((order) => order.id !== orderId ? order : {
          ...order,
          status: 'running',
          units: order.units.map((unit) => unit.id === unitId ? { ...unit, taskId, status: 'running' } : unit),
        }),
      })),
      failUnit: (orderId, unitId, error) => set((state) => ({
        orders: state.orders.map((order) => order.id !== orderId ? order : {
          ...order,
          status: order.units.some((unit) => unit.id !== unitId && (unit.status === 'done' || unit.status === 'running'))
            ? 'partially_failed'
            : 'failed',
          units: order.units.map((unit) => unit.id === unitId ? { ...unit, status: 'error', error } : unit),
        }),
      })),
      retryUnit: (orderId, unitId) => set((state) => ({
        orders: state.orders.map((order) => order.id !== orderId ? order : {
          ...order,
          status: 'queued',
          failedImages: Math.max(0, order.failedImages - (order.units.find((unit) => unit.id === unitId)?.quantity ?? 0)),
          units: order.units.map((unit) => unit.id === unitId
            ? { ...unit, status: 'queued', taskId: undefined, error: undefined }
            : unit),
        }),
      })),
      syncTasks: (tasks) => set((state) => {
        ...
        const orders = state.orders.map((order) => {
          ...
          const units: RequirementUnit[] = order.units.map((unit) => {
            ...
            if (!task) return unit
            const status: RequirementUnit['status'] = task.status === 'done' ? 'done' : task.status === 'error' ? 'error' : 'running'
          ...
          const allTerminal = units.every((unit) => unit.status === 'done' || unit.status === 'error' || unit.status === 'cancelled')
          const status: RequirementOrder['status'] = allTerminal
            ? failedImages === 0 ? 'completed' : completedImages > 0 ? 'partially_failed' : 'failed'
            : 'running'
          if (completedImages !== order.completedImages || failedImages !== order.failedImages || status !== order.status) changed = true
      ...
      cancelOrder: (orderId) => {
        ...
        set((state) => ({
          ...
          audits: user ? [{
            id: id('audit'), actorId: user.id, actorName: user.displayName, action: '取消需求',
            detail: state.orders.find((item) => item.id === orderId)?.number ?? orderId, createdAt: Date.now(),
          }, ...state.audits] : state.audits,
        }))
      },
      approveUrgent: (orderId, approved) => set((state) => ({
        orders: state.orders.map((order) => order.id === orderId ? {
          ...order,
      ...
      saveProduct: (product) => set((state) => ({
        catalog: { ...state.catalog, products: state.catalog.products.some((item) => item.id === product.id)
          ? state.catalog.products.map((item) => item.id === product.id ? product : item)
          : [...state.catalog.products, product] },
      })),
      saveChannel: (channel) => set((state) => ({
        catalog: { ...state.catalog, channels: state.catalog.channels.some((item) => item.id === channel.id)
          ? state.catalog.channels.map((item) => item.id === channel.id ? channel : item)
          : [...state.catalog.channels, channel] },
      })),
      saveMaterialType: (materialType) => set((state) => ({
        catalog: { ...state.catalog, materialTypes: state.catalog.materialTypes.some((item) => item.id === materialType.id)
      ...
      submitStrategyDraft: (materialTypeId) => set((state) => {
      ...
      publishStrategyDraft: (materialTypeId) => set((state) => {
        const user = state.users.find((item) => item.id === state.sessionUserId)
        const draft = state.strategyDrafts[materialTypeId]
        const current = state.catalog.materialTypes.find((item) => item.id === materialTypeId)
      ...
      rollbackMaterialType: (materialTypeId, version) => set((state) => {
        const user = state.users.find((item) => item.id === state.sessionUserId)
        const current = state.catalog.materialTypes.find((item) => item.id === materialTypeId)
        ...
        return {
          catalog: {
            ...state.catalog,
            materialTypes: state.catalog.materialTypes.map((item) => item.id === materialTypeId ? rolledBack : item),
          },
          strategyVersions: {
            ...state.strategyVersions,
            [materialTypeId]: [current, ...(state.strategyVersions[materialTypeId] ?? [])].slice(0, 20),
      ...
      saveStrategyAsset: (strategy) => set((state) => {
        ...
        return {
          strategyAssets: current
            ? state.strategyAssets.map((item) => item.id === normalized.id ? { ...normalized, updatedAt: Date.now() } : item)
            : [...state.strategyAssets, { ...normalized, updatedAt: Date.now() }],
          strategyAssetVersions: publishingNewVersion && current ? {
      ...
      moveStrategyAsset: (selectedStrategyId, productId, materialTypeId) => set((state) => ({
        strategyAssets: state.strategyAssets.map((item) => item.id === selectedStrategyId
          ? { ...item, productId, materialTypeId, updatedAt: Date.now() }
          : item),
      })),
      archiveStrategyAsset: (selectedStrategyId, archived = true) => set((state) => ({
        strategyAssets: state.strategyAssets.map((item) => item.id === selectedStrategyId
          ? { ...item, archived, updatedAt: Date.now() }
          : item),
      })),
      rollbackStrategyAsset: (selectedStrategyId, version) => set((state) => {
      ...
      saveStrategyPreset: (preset) => set((state) => ({
      ...
      archiveStrategyPreset: (presetId) => set((state) => ({
        strategyPresets: state.strategyPresets.map((item) => item.id === presetId ? { ...item, archived: true } : item),
      })),
      saveSopGroup: (group) => set((state) => ({
      ...
      duplicateSopGroup: (groupId) => {
        ...
        set((state) => ({
          sopGroups: [...state.sopGroups, { ...source, id: newId, name: `${source.name} 副本`, createdAt, updatedAt: createdAt }],
        }))
        return newId
      },
      deleteSopGroup: (groupId) => set((state) => ({
        sopGroups: state.sopGroups.filter((item) => item.id !== groupId),
        sopLibrary: state.sopLibrary.map((item) => item.groupId === groupId ? { ...item, groupId: undefined, updatedAt: Date.now() } : item),
      })),
      saveSopItem: (item) => set((state) => ({
      ...
      duplicateSopItem: (itemId) => {
        ...
        set((state) => ({
          sopLibrary: [{ ...source, id: newId, name: `${source.name} 副本`, source: 'manual', createdAt, updatedAt: createdAt }, ...state.sopLibrary],
        }))
        return newId
      },
      deleteSopItem: (itemId) => set((state) => ({
        sopLibrary: state.sopLibrary.filter((item) => item.id !== itemId),
      })),
      saveSopMetaInstruction: (item) => set((state) => ({
      ...
      duplicateSopMetaInstruction: (itemId) => {
        ...
        set((state) => ({
          sopMetaInstructions: [{ ...source, id: newId, name: `${source.name} 副本`, kind: 'custom', createdAt, updatedAt: createdAt }, ...state.sopMetaInstructions],
        }))
        return newId
      },
      deleteSopMetaInstruction: (itemId) => set((state) => ({
      ...
      addKnowledgeBatch: (batch) => {
        ...
        if (!user) return
        set((state) => ({
          knowledgeBatches: [{ ...batch, id: id('knowledge'), createdAt: Date.now(), createdBy: user.id }, ...state.knowledgeBatches],
        }))
      },
      updateKnowledgeBatch: (batchId, patch) => set((state) => ({
        knowledgeBatches: state.knowledgeBatches.map((batch) => batch.id === batchId ? { ...batch, ...patch } : batch),
      })),
      replaceKnowledgeInsights: (batchId, insights) => set((state) => ({
        knowledgeInsights: [
    ...
    }),
    {
      ...
      migrate: (persistedState) => {
        ...
        return {
          ...
          sopLibrary: state.sopLibrary?.length
            ? migrateSopLibraryExecutionModes(state.sopLibrary)
            : seedSopLibrary(state.strategyPresets ?? seedStrategyPresets()),
