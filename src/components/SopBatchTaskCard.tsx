...
function BatchThumbnail({ task, imageId, variantIndex, onClick }: { task: TaskRecord; imageId: string; variantIndex: number; onClick: () => void }) {
  ...
  return <button type="button" data-no-drag-select onClick={(event) => { event.stopPropagation(); onClick() }} aria-label={accessibleLabel} className="sop-batch-thumbnail relative min-h-0 overflow-hidden transition hover:brightness-90 focus-visible:outline-none">
...
export default function SopBatchTaskCard({
  ...
  return <Card onClick={onClick} data-selected={isSelected || undefined} className="gallery-task-card gallery-sop-card relative flex h-44 cursor-pointer overflow-hidden">
    ...
    <div className="gallery-task-media grid h-full w-40 min-w-[10rem] shrink-0 grid-cols-2 grid-rows-2 gap-px">
      {previews.map((preview) => <BatchThumbnail key={`${preview.task.id}-${preview.variantIndex}`} {...preview} onClick={onOpenBatch} />)}
    ...
    <div className="gallery-task-body flex min-w-0 flex-1 flex-col p-3">
      ...
      <div className="mt-auto flex items-center justify-between gap-2 pt-2"><Button type="button" data-no-drag-select onClick={(event) => { event.stopPropagation(); onOpenBatch() }} variant="ghost" size="sm" className="min-w-0 truncate">查看图片</Button><IconButton type="button" data-no-drag-select onClick={(event) => { event.stopPropagation(); onDelete() }} aria-label={`删除 SOP 批量任务 ${sopName}`} className="ds-icon-button--danger" size="sm" icon={<Trash2 size={16} />} /></div>
    </div>
  </Card>
}
