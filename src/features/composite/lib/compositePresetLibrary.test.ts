...
describe('composite preset library', () => {
  ...
  it('does not duplicate when the new preset id already exists', () => {
    ...
    const globalDuplicate = duplicatePresetIntoGroup(presets, createDefaultCompositeV2PresetGroup(1), preset.id, 'preset-copy', 3)
  ...
  it('filters presets by name and group membership', () => {
    ...
    expect(filterPresetsForLibrary(presets, groups, { query: '\u4ea7\u54c1', groupId: 'g1' }).map((preset) => preset.id)).toEqual([
      'b',
    ])
  })
