...
function drawTextLayer(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, layer: CompositeTextLayer) {
  ...
  if ((style.stroke.enabled && style.stroke.width > 0) || layer.strokeWidth > 0) ctx.strokeText(layer.text, x, y, rect.width)
  ctx.fillText(layer.text, x, y, rect.width)
}

function drawColorBlockLayer(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, layer: CompositeColorBlockLayer) {
  const rect = layerRect(layer, canvas)
  const style = getLayerStyle(layer)
  const scale = canvas.width / 1280
  applyShadow(ctx, style, scale)
