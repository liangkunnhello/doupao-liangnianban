...
function parseVariables(ruleBody: string): Record<string, string> {
  return Object.fromEntries(
    Array.from(ruleBody.matchAll(/--([\w-]+)\s*:\s*([^;]+);/g), (match) => [
      match[1],
      match[2].trim(),
    ]),
  )
...
function expectContrast(
  ...
  expect(
    ratio,
    `${id}/${mode}: ${foregroundName} on ${backgroundName} = ${ratio.toFixed(2)}:1`,
  ...
  it.each(customSkinIds)('%s meets the checked WCAG contrast pairs', (id) => {
    ...
    for (const mode of ['light', 'dark'] as const) {
      ...
      const gradientStops = Array.from(
        tokens['ds-color-primary-gradient'].matchAll(/hsl\(([^/)]+)\)/g),
        (match) => match[1].trim(),
      )
