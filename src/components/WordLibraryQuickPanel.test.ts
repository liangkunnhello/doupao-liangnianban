...
describe('filterWordLibraryEntries', () => {
  it('finds matches in names, candidate values, and tags while excluding trash', () => {
    ...
    expect(filterWordLibraryEntries({
      entries,
      query: '光影',
      view: 'all',
      groupId: '__all__',
    }).map((item) => item.id)).toEqual(['value-match', 'tag-match'])
  })
  ...
  it('keeps favorites focused and puts pinned entries first', () => {
    ...
    expect(filterWordLibraryEntries({
      ...
      view: 'favorites',
      groupId: '__all__',
    }).map((item) => item.id)).toEqual(['pinned-favorite', 'recent-favorite'])
  })

  it('limits the recent view after sorting by update time', () => {
    const entries = [
      entry('old', { updatedAt: 1 }),
      entry('middle', { updatedAt: 2 }),
      entry('new', { updatedAt: 3 }),
    ]
    ...
    expect(filterWordLibraryEntries({
      ...
      groupId: '__all__',
      limit: 2,
    }).map((item) => item.id)).toEqual(['new', 'middle'])
  })
})
