...
const COMPLIANCE_REWRITES: Array<[RegExp, string]> = [
  ...
  [/(100\s*%\s*(?:有效|安全|治愈|成功)?|百分之百(?:有效|安全|治愈|成功)?|零风险|永久有效|绝对安全|绝对有效|保证有效|保证治愈)/gi, '以实际情况为准'],
...
export function sanitizeInformationFlowAdResult(result: AssistantActionResult): AssistantActionResult {
  ...
  return {
    ...
    alternativePrompt: result.alternativePrompt ? sanitizeInformationFlowAdText(result.alternativePrompt) : result.alternativePrompt,
