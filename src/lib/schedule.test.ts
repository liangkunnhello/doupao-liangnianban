...
describe('schedule utilities', () => {
  ...
  it('treats all favorites as a filter instead of a source collection', () => {
    expect(resolveScheduleSourceCollectionId({
  ...
  it('selects due timed items in table order and one sequential untimed item per row', () => {
    const items: ScheduleItem[] = [
      { id: 'timed-late', taskId: 'task', collectionId: null, date: '2026-06-18', rowId: 'row-2', order: 0, count: 1, time: '08:00' },
      { id: 'timed-first', taskId: 'task', collectionId: null, date: '2026-06-18', rowId: 'row-1', order: 0, count: 1, time: '08:00' },
      { id: 'seq-done', taskId: 'task', collectionId: null, date: '2026-06-18', rowId: 'row-3', order: 0, count: 1, time: null, status: 'done', lastRunKey: '2026-06-18:seq-done' },
      { id: 'seq-next', taskId: 'task', collectionId: null, date: '2026-06-18', rowId: 'row-3', order: 1, count: 1, time: null },
      { id: 'seq-waiting', taskId: 'task', collectionId: null, date: '2026-06-18', rowId: 'row-4', order: 1, count: 1, time: null },
      { id: 'seq-running', taskId: 'task', collectionId: null, date: '2026-06-18', rowId: 'row-4', order: 0, count: 1, time: null, status: 'running', lastRunKey: '2026-06-18:seq-running' },
    ]
  ...
  it('asks for a supplement run when terminal scheduled tasks produce fewer images than requested', () => {
    ...
    expect(getScheduleCompletionAction(item, [
      { id: 'run-a', status: 'done', outputImages: Array.from({ length: 40 }, (_, index) => `img-${index}`), error: null },
  ...
  it('completes scheduled tasks without supplement when output count reaches the requested count', () => {
    ...
    expect(getScheduleCompletionAction(item, [
      { id: 'run-a', status: 'done', outputImages: Array.from({ length: 51 }, (_, index) => `img-${index}`), error: null },
    ])).toEqual({ type: 'done' })
  })
