...
describe('assistant word entry groups', () => {
  it('always creates a fresh group in "new" mode, never reusing a same-named one', () => {
    ...
    const groupId = resolveAssistantWordGroupId(baseOptions, [
  ...
  it('keeps an explicitly selected group in "selected" mode', () => {
    const createGroup = vi.fn(() => 'new-group')
    const groupId = resolveAssistantWordGroupId({ ...baseOptions, targetGroupMode: 'selected', targetGroupId: 'fixed' }, [
  ...
  it('falls back to a new group when "selected" target is missing', () => {
    const createGroup = vi.fn(() => 'new-group')
    const groupId = resolveAssistantWordGroupId({ ...baseOptions, targetGroupMode: 'selected', targetGroupId: 'ghost' }, [
  ...
  it('builds a name from skill, prompt summary and timestamp', () => {
    expect(buildWordGroupName('爆款衍生', '防晒霜夏季主视觉方案长文本用于验证截断逻辑', 0))
      .toMatch(/^爆款衍生 · 防晒霜夏季主视觉方案长文 · \d{2}-\d{2} \d{2}:\d{2}$/)
  })
