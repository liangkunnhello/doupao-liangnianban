...
import { renderCombinedOverlay } from '../lib/compositeRendererV2'
...
function getCenteredFreePosition(
  baseCanvas: CompositeV2Preset['baseCanvas'],
  ...
  async function openImagePicker(layer: CompositeV2Layer, event?: React.MouseEvent<HTMLButtonElement>) {
    ...
    img.onload = () => {
      ...
      updateLayer(layer.id, {
        ...
        position: layer.position.mode === 'free'
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-gray-100 dark:bg-gray-950">
      <div ref={stageRef} data-layout="preset-canvas-stage" className="relative min-h-0 flex-1 overflow-hidden">
        <FloatingLayerToolbar onAddText={props.onAddText} onAddImage={props.onAddImage} onAddLogo={props.onAddLogo} disabled={!preset} />
        {preset && (
          <div data-layout="preset-title" className="absolute left-3 top-2 z-10 rounded bg-white/85 px-2 py-1 text-xs font-medium dark:bg-gray-950/85">
            {preset.name}
          </div>
        )}
        ...
        <div ref={containerRef} className="h-full w-full overflow-auto">
          <div className="flex min-h-full min-w-full p-8">
            <div
              ...
              className={`relative m-auto overflow-hidden rounded-md border border-gray-300 shadow-inner dark:border-white/[0.08] ${getPreviewBackdropClasses(previewBackdropMode)}`}
              style={{
      ...
      {/* Zoom Controls Bar */}
      <div className="flex h-10 shrink-0 items-center justify-between border-t border-gray-200 bg-white px-4 dark:border-white/[0.08] dark:bg-gray-950">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            onClick={() => setPreviewBackdropMode((current) => previewBackdropOrder[(previewBackdropOrder.indexOf(current) + 1) % previewBackdropOrder.length]!)}
            className="mr-2 rounded-md border border-gray-200 px-2 py-1 text-xs text-gray-600 transition hover:bg-gray-100 hover:text-gray-900 dark:border-white/[0.08] dark:text-gray-300 dark:hover:bg-white/[0.06] dark:hover:text-gray-100"
          >
          ...
          <button
            ...
            className="min-w-[60px] text-center text-xs font-medium text-gray-700 hover:text-blue-600 dark:text-gray-200 dark:hover:text-blue-400"
          >
