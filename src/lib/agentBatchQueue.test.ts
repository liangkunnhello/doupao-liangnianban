...
function plan(): AgentBatchPlan {
  const unit = (id: string, date: string, plannedCount: number) => ({
    id, sourceId: 'row', date, sku: 'S', product: 'P', channel: 'C', specification: '1:1', direction: 'D',
    strategy: 'S', copyMode: 'without-copy' as const, targetCount: plannedCount, plannedCount,
    outputFolder: 'D:/out', prompt: 'prompt',
  })
  return {
    targetCount: 5, plannedCount: 5, redundancyCount: 0,
    days: [
