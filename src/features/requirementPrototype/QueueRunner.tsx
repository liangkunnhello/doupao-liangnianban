...
export default function RequirementQueueRunner() {
  ...
  useEffect(() => {
    for (const order of orders) {
      ...
      void writeRequirementManifests(order, catalog, tasks).then((written) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const inputImages = (await Promise.all((unit.referenceImageIds ?? []).map(async (imageId) => {
        ...
        useRequirementPrototype.getState().failUnit(
          order.id,
          unit.id,
