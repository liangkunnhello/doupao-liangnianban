...
import { appendAdNegativeRule, createAdNegativeRuleSnapshot, getAdNegativeRule, normalizeAdNegativeRuleProfiles } from './adNegativeRules'
...
describe('ad negative rules', () => {
  it('always restores the three built-in profiles and preserves valid custom profiles', () => {
    const profiles = normalizeAdNegativeRuleProfiles([
      { id: 'custom-1', name: '自定义', description: '测试', content: '不得生成二维码', source: 'custom', platform: 'custom', version: 2, updatedAt: 1 },
    ])
    expect(profiles.slice(0, 3).map((rule) => rule.id)).toEqual(DEFAULT_AD_NEGATIVE_RULE_PROFILES.map((rule) => rule.id))
