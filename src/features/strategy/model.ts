...
function normalizeReference(input: LegacyStrategyAsset): StrategyReferenceConfig | undefined {
  ...
  const source = legacyReference.sourceType === 'knowledge-material'
...
export function normalizeStrategyAsset(input: LegacyStrategyAsset): StrategyAsset {
  ...
  const normalizedMode = input.generationMode === 'image-to-image' || (input.generationMode === 'sop' && reference)
    ? 'image-to-image'
    : input.generationMode === null
      ? null
      : 'text-to-image'
  const legacyInsightIds = input.steps
  ...
  return [
    {
      ...
      value: '先确认产品事实与渠道禁用项，再确定单一传播目标；画面信息不超过三层，不伪造系统界面、通知、中奖或权威背书；批量结果必须主动改变构图、场景和视觉焦点。',
      global: true,
...
export function createStrategyAsset(
  productId: string,
...
export function presetLabel(type: StrategyPresetType) {
  if (type === 'sop') return 'SOP 预设'
  if (type === 'export') return '渠道导出预设'
  return '输出分配预设'
}
...
export function validateStrategyForTest(strategy: StrategyAsset, materialType?: CatalogMaterialType) {
  ...
  if (strategy.outputs.channels.enabled && strategy.outputs.channels.channelIds.length === 0) errors.push('已启用输出渠道，请至少选择一个渠道')
  if (strategy.outputs.sizes.enabled && strategy.outputs.sizes.ratios.length === 0) errors.push('已启用输出尺寸，请至少选择一个尺寸')
  if (strategy.outputs.export.enabled) {
    if (!strategy.outputs.channels.enabled || strategy.outputs.channels.channelIds.length === 0) errors.push('渠道导出需要先启用并选择输出渠道')
    if (!strategy.outputs.export.presetId) errors.push('已启用渠道导出，请选择导出预设')
  }
  if (strategy.outputs.allocation.enabled) {
    if (!strategy.outputs.channels.enabled && !strategy.outputs.sizes.enabled) errors.push('输出分配需要先启用输出渠道或输出尺寸')
...
export function buildStrategyFlowSteps(strategy: StrategyAsset, catalog: RequirementCatalog): StrategyFlowStep[] {
  ...
  if (strategy.workflow.knowledge.resolved) {
    ...
    steps.push({ id: 'knowledge', kind: 'knowledge', label: count ? `已选择 ${count} 条知识` : '不使用知识库', value: String(count) })
  }
  ...
  if (strategy.outputs.sizes.enabled) {
    steps.push({ id: 'size', kind: 'size', label: strategy.outputs.sizes.ratios.join('、'), value: strategy.outputs.sizes.ratios.join(',') })
  }
  if (strategy.outputs.export.enabled) {
    steps.push({ id: 'export', kind: 'export', label: '已启用渠道导出', value: strategy.outputs.export.presetId ?? '' })
  }
  if (strategy.outputs.allocation.enabled) {
    steps.push({ id: 'allocation', kind: 'allocation', label: '已启用输出分配', value: strategy.outputs.allocation.presetId ?? '' })
  }
  return steps
}
