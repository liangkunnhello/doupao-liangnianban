...
function buildXlsx(rows: Array<Array<string | number>>) {
  ...
  const files = {
    ...
    'xl/workbook.xml': strToU8('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="生成概览" sheetId="1" r:id="rId1"/></sheets></workbook>'),
...
export async function writeRequirementManifests(
  ...
  const htmlRows = rows.map((row) => `<tr>${Object.values(row).slice(0, 10).map((value) => `<td>${escapeHtml(value)}</td>`).join('')}</tr>`).join('')
  ...
  const outputPaths = [...new Set(order.units
  ...
  await Promise.all(outputPaths.map(async (outputPath) => {
    ...
    await Promise.all([
      ...
      api.saveZipBuffer(xlsxPath, xlsx.buffer.slice(xlsx.byteOffset, xlsx.byteOffset + xlsx.byteLength) as ArrayBuffer),
    ])
  }))
  return true
}
