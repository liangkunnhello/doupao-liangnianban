...
export function composeOrderingPrompt(
  ...
  const strategy = materialType.mode === 'fixed'
    ? [
...
function getCombinationError(
  ...
  if (materialType.compatibleProductIds && !materialType.compatibleProductIds.includes(product.id)) return '素材类型与产品不兼容'
  if (materialType.compatibleChannelIds && !materialType.compatibleChannelIds.includes(channel.id)) return '素材类型与渠道不兼容'
  if (materialType.mode === 'fixed' && !materialType.supportedRatios?.includes(ratio)) return `固定规则缺少 ${ratio} 适配`
...
export function planOrderingOrder(
  ...
  if (draft.channels.length === 0 || draft.channels.every((item) => item.ratios.length === 0)) errors.push('请选择至少一个渠道规格')
  ...
  if (totalImages > limits.maxImagesPerOrder) errors.push(`计划 ${totalImages} 张，超过单张需求上限 ${limits.maxImagesPerOrder} 张`)
  if (totalImages > limits.remainingDailyQuota) errors.push(`计划 ${totalImages} 张，超过今日剩余额度 ${limits.remainingDailyQuota} 张`)
