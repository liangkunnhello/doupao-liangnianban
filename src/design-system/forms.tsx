...
export interface SwitchProps
  extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type' | 'onChange' | 'role'> {
  checked: boolean
    ...
    <fieldset className={cx('ds-radio-group', className)} disabled={disabled}>
      ...
      <div className={cx('ds-radio-group__options', `ds-radio-group__options--${orientation}`)}>
        {options.map((option) => (
          <label
            key={option.value}
            className={cx('ds-radio', option.disabled && 'ds-radio--disabled')}
          >
            <input
              type="radio"
              name={groupName}
            ...
            <span className="ds-radio__copy">
              ...
              {option.description && (
                <span className="ds-radio__description">{option.description}</span>
              )}
            </span>
          </label>
        ))}
...
export const SearchField = forwardRef<HTMLInputElement, SearchFieldProps>(function SearchField(
  { className, id: providedId, label, onChange, onClear, value, ...props },
  ref,
) {
  const generatedId = useId()
  const id = providedId ?? generatedId
