    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="shrink-0 space-y-2.5 border-b border-border px-4 py-3">
        ...
        <label className="block">
          ...
          <select
            ...
            className="h-9 w-full rounded-md border border-border bg-background px-2.5 text-sm outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          >
      ...
      >
        {visibleEntries.map((entry) => {
          ...
          return (
            <div
              ...
              className={cx(
                'word-entry-row group flex min-h-[52px] items-center gap-2 rounded-lg px-2 py-1.5',
            ...
            >
              ...
              >
                <span className="word-entry-initial h-8 w-8 text-sm">{entry.key.slice(0, 1) || '词'}</span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-medium text-foreground">{entry.key}</span>
        ...
        {visibleEntries.length === 0 && (
          <EmptyState
            ...
            description={view === 'favorites' && !query ? '在“最近”或“全部”中收藏常用词条。' : '调整搜索词或分组后再试。'}
            action={view === 'favorites' && !query ? (
              <Button size="sm" variant="secondary" onClick={() => onViewChange('all')}>查看全部</Button>
            ) : undefined}
          />
        )}
      </div>
      ...
      >
        {activeEntry ? (
          <>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-foreground">{activeEntry.key}</p>
            ...
            {!derivativeOpen && (
              <div className="mt-2 max-h-28 space-y-1 overflow-y-auto rounded-md border border-border bg-muted/30 p-2">
                {activeEntry.entries.slice(0, 3).map((value, index) => (
                  <p key={`${value}-${index}`} className="break-words text-xs leading-5 text-foreground">
                    {value}
                  </p>
                ))}
                {activeEntry.entries.length === 0 && (
                  <p className="text-xs text-muted-foreground">这个词条还没有候选值。</p>
                )}
                {activeEntry.entries.length > 3 && (
                  <p className="text-xs text-muted-foreground">还有 {activeEntry.entries.length - 3} 条</p>
                )}
              </div>
            )}
              ...
              <div className="mt-4 border-t border-border pt-4">
                <label className="block text-xs text-muted-foreground">
                  ...
                  <textarea
                    ...
                    className="mt-1 min-h-32 w-full resize-y rounded-md border border-border bg-background p-2 text-sm leading-6 text-foreground"
                  />
                </label>
          ...
          <div className="py-3 text-center text-xs text-muted-foreground">选择词条后在这里预览对应词库。</div>
        )}
      </section>
    </div>
  )
