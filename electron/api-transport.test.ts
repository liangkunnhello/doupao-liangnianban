...
describe('main-process API transport request validation', () => {
  ...
  it('streams Node fetch response chunks back to the requesting renderer', async () => {
    registerApiTransport()
    const handleMock = vi.mocked(ipcMain.handle)
    const handler = handleMock.mock.calls.find(([channel]) => channel === 'api:fetch')?.[1]
    expect(handler).toBeTypeOf('function')
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('main-process-body', {
      status: 201,
    ...
    const metadata = await handler!({ sender } as never, {
      id: 'request-stream',
