...
export function getPromptRunImageLinks(run: SopBatchSnapshot, tasks: TaskRecord[]) {
  ...
  return tasks.flatMap((task) => {
    const isLinked = task.sopBatch?.snapshotId
      ? task.sopBatch.snapshotId === run.id
      : knownTaskIds.has(task.id)
    if (!isLinked) return []

    const promptId = task.sopBatch?.promptId
      ?? orderedPromptIds[Math.max(0, (task.sopBatch?.promptIndex ?? 1) - 1)]
