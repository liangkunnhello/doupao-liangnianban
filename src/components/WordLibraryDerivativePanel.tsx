...
export default function WordLibraryDerivativePanel({
  ...
  const ruleSummary = enabledRules.length === 0
    ? '默认规则'
  ...
  const toggleRule = (ruleId: string) => {
    const rules = settings.wordLibraryDerivativeRules
    const next = settings.wordLibraryDerivativeRuleMode === 'single'
  ...
  const addRule = () => {
    ...
    updateRules([...settings.wordLibraryDerivativeRules, {
      id,
      name: '新规则',
  ...
  const patchRule = (ruleId: string, patch: Partial<WordLibraryDerivativeRule>) => {
    updateRules(settings.wordLibraryDerivativeRules.map((rule) => (
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <span className="mt-0.5 text-blue-500" aria-hidden="true"><SparklesIcon className="h-4 w-4" /></span>
        <div className="min-w-0 flex-1">
          <h3 id="word-library-derivative-title" className="text-sm font-semibold">AI 衍生词条</h3>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">分析当前全部候选值，生成可替换同一变量的新词条。</p>
        </div>
      </div>

      <div className="mt-3 grid grid-cols-[1fr_76px] gap-3">
        <label className="min-w-0 text-xs text-muted-foreground">
          <span className="flex items-center justify-between"><span>相似度</span><output>{similarity}%</output></span>
          <input
            type="range"
            min="0"
            max="100"
      ...
      <div className="mt-3 flex items-center justify-between gap-2 text-xs">
        <span className="min-w-0 truncate text-muted-foreground" title={ruleSummary}>规则：{ruleSummary}</span>
        <button type="button" className="shrink-0 text-blue-600 hover:underline" onClick={() => setRulesOpen((open) => !open)}>
          {rulesOpen ? '收起规则' : '管理规则'}
        </button>
      </div>
      ...
      {rulesOpen && (
        <div className="mt-3 border-t border-border pt-3">
          <div className="flex items-center justify-between gap-2">
            ...
            />
            <Button size="sm" variant="secondary" onClick={addRule}>添加规则</Button>
          </div>
          <div className="mt-3 space-y-2">
            {settings.wordLibraryDerivativeRules.map((rule) => {
              const editing = editingRuleId === rule.id && !rule.builtIn
              return (
                <article key={rule.id} className={`rounded-md border p-2.5 ${rule.enabled ? 'border-blue-500/50 bg-blue-500/10' : 'border-border bg-background'}`}>
                  <div className="flex items-start gap-2">
                    <input
                      ...
                      {editing ? (
                        <div className="space-y-2">
                          ...
                          <textarea
                            ...
                            aria-label="规则内容"
                            className="min-h-24 w-full resize-y rounded-md border border-border bg-background p-2 text-xs leading-5 text-foreground"
                          />
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center gap-2">
                            ...
                            {rule.builtIn && <span className="shrink-0 text-[10px] text-blue-500">默认</span>}
                          </div>
                          <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{rule.content}</p>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="mt-2 flex justify-end gap-2">
                    {editing ? (
                      <button type="button" className="text-xs text-blue-600" onClick={() => setEditingRuleId(null)}>完成</button>
                    ) : (
                      <>
                        {!rule.builtIn && <button type="button" className="text-xs text-muted-foreground hover:text-foreground" onClick={() => setEditingRuleId(rule.id)}>编辑</button>}
                        <button type="button" className="text-xs text-muted-foreground hover:text-foreground" onClick={() => copyRule(rule)}>复制</button>
                        {!rule.builtIn && <button type="button" className="text-xs text-red-500" onClick={() => deleteRule(rule.id)}>删除</button>}
                      </>
                    )}
                  </div>
                </article>
        ...
        <div className="mt-3" aria-live="polite">
          <div className="max-h-40 overflow-y-auto rounded-md border border-border bg-background p-2">
            {generatedEntries.map((entry, index) => (
              <p key={`${entry}-${index}`} className="border-b border-border py-1.5 text-xs leading-5 last:border-b-0">{entry}</p>
            ))}
          </div>
          ...
          <button type="button" className="mt-2 w-full text-center text-xs text-muted-foreground hover:text-foreground" onClick={() => setGeneratedEntries([])}>丢弃结果</button>
        </div>
      )}
    </section>
  )
