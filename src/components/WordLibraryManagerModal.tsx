...
export default function WordLibraryManagerModal() {
  ...
  const visibleGroupIds = useMemo(() => groupId === '__all__' ? null : new Set([groupId, ...activeGroups.filter((group) => group.parentId === groupId).map((group) => group.id)]), [activeGroups, groupId])
  const visibleEntries = useMemo(() => {
    const needle = query.trim().toLowerCase()
    return activeEntries.filter((entry) => (
      (!visibleGroupIds || visibleGroupIds.has(entry.groupId))
      && (!needle || entry.key.toLowerCase().includes(needle) || entry.entries.some((value) => value.toLowerCase().includes(needle)) || entry.tags.some((tag) => tag.toLowerCase().includes(needle)))
    ))
  ...
  const saveGroup = () => {
    ...
    if (activeGroups.some((group) => group.name === name && (group.parentId ?? null) === groupForm.parentId)) return toast('同一层级下已存在同名分组', 'error')
    const group = createGroup(name, groupForm.parentId)
    setGroupId(group.id); setGroupForm(null)
  }
  const saveEntry = () => {
  ...
  const requestDelete = () => {
    ...
    confirm({
      ...
      action: () => { deleteEntry(activeEntry.id); setEntryId(null) },
    })
  }
  ...
  const clearEntrySelection = () => { setSelectedEntryIds([]); setSelectionAnchorId(null) }
  const selectVisibleEntries = () => { setSelectedEntryIds(visibleEntries.map((entry) => entry.id)); setSelectionAnchorId(visibleEntries[0]?.id ?? null) }
  const startLassoSelection = (event: ReactPointerEvent<HTMLElement>) => {
    if (event.button !== 0 || (event.target as HTMLElement).closest('[data-entry-id], button, input, select, label, textarea')) return
    const bounds = entryListRef.current?.getBoundingClientRect()
    if (!bounds) return
    const additive = event.ctrlKey || event.metaKey
    lassoStartRef.current = { x: event.clientX, y: event.clientY, initialIds: selectedEntryIds, additive }
    event.currentTarget.setPointerCapture(event.pointerId)
    setLasso({ x: event.clientX - bounds.left, y: event.clientY - bounds.top, width: 0, height: 0 })
  }
  const moveLassoSelection = (event: ReactPointerEvent<HTMLElement>) => {
    ...
    const ids = [...(entryListRef.current?.querySelectorAll<HTMLElement>('[data-entry-id]') ?? [])]
      .filter((node) => {
    ...
    setSelectedEntryIds(start.additive ? [...new Set([...start.initialIds, ...ids])] : ids)
    setLasso({ x: left - bounds.left, y: top - bounds.top, width: right - left, height: bottom - top })
  }
  const endLassoSelection = (event: ReactPointerEvent<HTMLElement>) => {
  ...
  const requestBatchDelete = () => {
    ...
    confirm({
      ...
      confirmText: '删除', tone: 'danger',
      action: () => { batchDeleteEntries(selectedEntryIds); clearEntrySelection(); setEntryId(null); toast('词条已移入回收站', 'success') },
    })
  }
  const applyBatchMove = () => {
    if (!selectedEntryIds.length || !batchTargetGroupId) return
    batchMoveEntries(selectedEntryIds, batchTargetGroupId); clearEntrySelection(); toast('词条已移动', 'success')
  }
  const applyBatchTags = () => {
    const tags = batchTags.split(/[,，\n]/).map((tag) => tag.trim()).filter(Boolean)
    if (!selectedEntryIds.length || !tags.length) return
    batchAddTags(selectedEntryIds, tags); setBatchTags(''); toast('标签已添加', 'success')
  }
  const applyBatchDuplicate = () => {
    if (!selectedEntryIds.length) return
    const count = duplicateEntries(selectedEntryIds); clearEntrySelection(); toast(`已复制 ${count} 个词条`, 'success')
  }
  const applyBatchPin = () => {
    if (!selectedEntryIds.length) return
    batchPinEntries(selectedEntryIds); clearEntrySelection(); toast('词条已置顶', 'success')
  }
  const requestDeleteGroup = () => {
    ...
    const target = activeGroups.find((group) => group.id !== selectedGroup.id && group.name === '默认分组')
      ?? activeGroups.find((group) => group.id !== selectedGroup.id)
    if (!target) return
    confirm({
      title: `删除分组「${selectedGroup.name}」？`,
      message: entryCount ? `其中 ${entryCount} 个词条会移动到「${target.name}」，子分组将转为顶级分组。` : '子分组将转为顶级分组。',
      confirmText: '删除分组', tone: 'danger',
      ...
      <section className="flex h-[min(820px,92vh)] w-[min(1320px,96vw)] flex-col overflow-hidden rounded-lg border border-border bg-background shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {view === 'library' && <div className="grid min-h-0 flex-1 grid-cols-[240px_minmax(360px,1fr)_340px]">
          <aside className="overflow-y-auto border-r border-border p-3">
            ...
            <div className="mb-2 flex items-center justify-between px-2 text-xs text-muted-foreground"><span>分组</span><button type="button" onClick={() => setGroupForm({ parentId: null, name: '' })} className="text-blue-600">新建</button></div>
            {groupForm && <div className="mb-2 rounded-md border border-border p-2"><input autoFocus value={groupForm.name} onChange={(event) => setGroupForm({ ...groupForm, name: event.target.value })} placeholder="分组名称" className="h-8 w-full rounded border border-border bg-muted/30 px-2 text-sm" /><div className="mt-2 flex gap-2"><button type="button" onClick={saveGroup} className="text-xs text-blue-600">创建</button><button type="button" onClick={() => setGroupForm(null)} className="text-xs text-muted-foreground">取消</button></div></div>}
            {roots.map((root) => <div key={root.id} className="mb-1"><div className={`group flex rounded-md ${groupId === root.id ? 'bg-blue-500/15 text-blue-600' : 'hover:bg-muted'}`}><button type="button" onClick={() => setGroupId(root.id)} className="min-w-0 flex-1 truncate px-3 py-2 text-left text-sm">{root.name}<span className="ml-2 text-xs text-muted-foreground">{countGroup(root.id)}</span></button><button type="button" onClick={() => setGroupForm({ parentId: root.id, name: '' })} className="mr-1 hidden h-7 w-7 group-hover:block" title="新建子分组">+</button></div>{activeGroups.filter((group) => group.parentId === root.id).map((child) => <button key={child.id} type="button" onClick={() => setGroupId(child.id)} className={`ml-4 flex w-[calc(100%-1rem)] rounded-md px-3 py-1.5 text-left text-sm ${groupId === child.id ? 'bg-blue-500/15 text-blue-600' : 'hover:bg-muted'}`}>└ {child.name}<span className="ml-auto text-xs text-muted-foreground">{countGroup(child.id)}</span></button>)}</div>)}
          </aside>
          <main ref={entryListRef} className="relative select-none overflow-y-auto p-3" onPointerDown={startLassoSelection} onPointerMove={moveLassoSelection} onPointerUp={endLassoSelection} onPointerCancel={endLassoSelection}>
            {selectedGroup && <div className="mb-3 flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/30 p-2 text-xs"><input value={groupNameDraft || selectedGroup.name} onChange={(event) => setGroupNameDraft(event.target.value)} className="h-7 min-w-28 flex-1 rounded border border-border bg-background px-2" aria-label="分组名称" /><button type="button" onClick={() => { const name = (groupNameDraft || selectedGroup.name).trim(); if (!name) return; renameGroup(selectedGroup.id, name); setGroupNameDraft(''); toast('分组已重命名', 'success') }} className="rounded border border-border px-2 py-1">重命名</button><select value={mergeTargetId} onChange={(event) => setMergeTargetId(event.target.value)} className="h-7 max-w-32 rounded border border-border bg-background px-1"><option value="">合并到…</option>{activeGroups.filter((group) => group.id !== selectedGroup.id).map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select><button type="button" disabled={!mergeTargetId} onClick={() => { mergeGroups(selectedGroup.id, mergeTargetId); setGroupId(mergeTargetId); setMergeTargetId(''); toast('分组已合并', 'success') }} className="rounded border border-border px-2 py-1 disabled:opacity-40">合并</button><button type="button" onClick={() => { archiveGroup(selectedGroup.id); setGroupId('__all__'); toast('分组已归档', 'success') }} className="rounded border border-border px-2 py-1">归档</button><button type="button" onClick={requestDeleteGroup} className="rounded border border-red-500/40 px-2 py-1 text-red-500 disabled:opacity-40" disabled={activeGroups.length <= 1}>删除</button></div>}
            <div className="mb-2 flex items-center justify-between text-sm"><span className="font-medium">{selectedGroup?.name ?? '全部词条'}</span><span className="text-muted-foreground">{visibleEntries.length} 条</span></div>
            <div className="mb-3 rounded-md border border-border bg-muted/20 p-2 text-xs"><div className="flex items-center gap-2"><label className="flex items-center gap-1.5"><input type="checkbox" checked={visibleEntries.length > 0 && visibleEntries.every((entry) => selectedEntryIds.includes(entry.id))} onChange={(event) => event.target.checked ? selectVisibleEntries() : clearEntrySelection()} /> 全选当前结果</label><span className="text-muted-foreground">已选 {selectedEntryIds.length} 条</span>{selectedEntryIds.length > 0 && <button type="button" onClick={clearEntrySelection} className="ml-auto text-muted-foreground hover:text-foreground">取消选择</button>}</div><p className="mt-1 text-muted-foreground">单击选择；Ctrl/⌘ 切换选择；Shift 连续多选；在列表空白区域拖拽可框选。</p>{selectedEntryIds.length > 0 && <div className="mt-2 flex flex-wrap items-center gap-2"><select value={batchTargetGroupId} onChange={(event) => setBatchTargetGroupId(event.target.value)} className="h-7 rounded border border-border bg-background px-1"><option value="">移动到分组…</option>{activeGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select><button type="button" disabled={!batchTargetGroupId} onClick={applyBatchMove} className="rounded border border-border px-2 py-1 disabled:opacity-40">移动</button><input value={batchTags} onChange={(event) => setBatchTags(event.target.value)} placeholder="标签，逗号分隔" className="h-7 w-28 rounded border border-border bg-background px-2" /><button type="button" onClick={applyBatchTags} className="rounded border border-border px-2 py-1">加标签</button><button type="button" onClick={applyBatchPin} className="rounded border border-border px-2 py-1">置顶</button><button type="button" onClick={applyBatchDuplicate} className="rounded border border-border px-2 py-1">复制</button><button type="button" onClick={requestBatchDelete} className="rounded border border-red-500/40 px-2 py-1 text-red-500">删除</button></div>}</div>
            {visibleEntries.map((entry) => <div key={entry.id} data-entry-id={entry.id} className={`mb-1.5 flex w-full items-center gap-2 rounded-md border p-3 text-left ${selectedEntryIds.includes(entry.id) ? 'border-blue-500/60 bg-blue-500/10' : entry.id === entryId ? 'border-blue-500/50 bg-blue-500/10' : 'border-border hover:bg-muted/50'}`}><input type="checkbox" checked={selectedEntryIds.includes(entry.id)} onClick={(event) => { event.stopPropagation(); selectEntryWithModifiers(entry.id, event) }} onChange={() => undefined} aria-label={`选择词条 ${entry.key}`} /><button type="button" onClick={(event) => selectEntryWithModifiers(entry.id, event)} className="flex min-w-0 flex-1 items-center text-left"><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{entry.key}</span><span className="mt-1 block truncate text-xs text-muted-foreground">{activeGroups.find((group) => group.id === entry.groupId)?.name ?? '未分组'} · {entry.entries.length} 个候选值 · 引用 {referenceCounts.get(entry.id) ?? 0}</span></span><span className="text-xs text-muted-foreground">使用 {entry.usageCount}</span></button></div>)}
