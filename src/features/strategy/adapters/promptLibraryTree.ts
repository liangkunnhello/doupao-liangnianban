...
export function normalizePromptLibraryFolders(raw: unknown, runs: SopBatchSnapshot[]) {
  ...
  parsed.forEach((item, index) => {
    ...
    folders.push({
      id,
      name,
      parentId: typeof candidate.parentId === 'string' && candidate.parentId !== id
        ? candidate.parentId
        : null,
...
export function movePromptLibraryFolder(
  ...
  const oldSiblings = oldParentId === targetParentId
    ? []
    : getSortedFolderChildren(folders.filter((folder) => folder.id !== source.id), oldParentId)
...
export function movePromptLibraryFolderToParent(
  ...
  const siblings = getSortedFolderChildren(folders.filter((folder) => folder.id !== sourceId), parentId)
  const orderById = new Map(siblings.map((folder, index) => [folder.id, index]))
  const oldSiblings = getSortedFolderChildren(folders.filter((folder) => folder.id !== sourceId), source.parentId)
  oldSiblings.forEach((folder, index) => orderById.set(folder.id, index))
  return folders.map((folder) => {
