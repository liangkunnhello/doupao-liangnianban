...
function isQueue(value: unknown): value is AgentBatchQueue {
  return Boolean(value) && typeof value === 'object' && (value as AgentBatchQueue).version === 2 &&
    Array.isArray((value as AgentBatchQueue).plan?.days) && typeof (value as AgentBatchQueue).id === 'string'
}
...
function migrateLegacyQueue(queue: LegacyAgentBatchQueue): AgentBatchQueue {
  return {
    ...
    receipt: {
      planName: '已迁移批量计划', outputRoot: '', apiProfileName: '未知配置', apiModel: '未知模型', maxConcurrent: 0, execution: 'scheduled',
    },
...
export function createAgentBatchQueue(
  ...
  const receipt: AgentBatchQueueReceipt = typeof receiptOrNow === 'number'
    ? { planName: '未命名批量任务', outputRoot: '', apiProfileName: '未知配置', apiModel: '未知模型', maxConcurrent: 0, execution: 'scheduled' }
    : receiptOrNow
...
export function updateAgentBatchQueue(queue: AgentBatchQueue, storage: StorageLike | null = getStorage()) {
  ...
  saveAgentBatchQueues(index < 0 ? [...queues, queue] : queues.map((item) => item.id === queue.id ? queue : item), storage)
}
...
export function getAgentBatchQueueStatusLabel(status: AgentBatchQueueStatus) {
  return ({ waiting: '等待执行', running: '正在提交', paused: '已暂停', completed: '已全部提交', failed: '提交失败', cancelled: '已取消' } as const)[status]
}
