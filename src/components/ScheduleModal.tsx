...
import { formatDateKey, getWeekDates, getWeekStartDate, parseDateKey, resolveScheduleSourceCollectionId } from '../lib/schedule'
...
export default function ScheduleModal() {
  ...
  const getCellItems = (dateKey: string, rowId: string) => schedule.items
    .filter((item) => item.date === dateKey && item.rowId === rowId)
  ...
  const handleDrop = (event: React.DragEvent, date: string, rowId: string) => {
    ...
    try {
      ...
      if (payload.scheduleItemId) {
        ...
        const targetItems = schedule.items.filter((item) =>
  ...
  const handleRemoveRow = (row: ScheduleRow) => {
    ...
    setConfirmDialog({
      ...
      cancelText: '取消',
      tone: 'danger',
      action: () => removeScheduleRow(row.id),
    })
  }
      ...
      <div ref={modalRef} className="flex h-[88vh] w-[min(1280px,96vw)] flex-col overflow-hidden rounded-xl border border-border bg-background text-foreground shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden text-xs text-muted-foreground md:block">{todayStatusText}</div>
            <button
              type="button"
              onClick={toggleScheduleWeek}
              className={`rounded-md border px-3 py-1.5 text-xs ${todayRunning ? 'border-red-500/30 text-red-500 hover:bg-red-500/10' : 'border-blue-500/30 text-blue-600 hover:bg-blue-500/10 dark:text-blue-300'}`}
            >
              {todayRunning ? '\u505c\u6b62\u4efb\u52a1' : '\u542f\u52a8\u4efb\u52a1'}
            </button>
            <button type="button" onClick={() => setScheduleWeekStart(addDays(schedule.activeWeekStart, -7))} className="rounded-md border border-border px-3 py-1.5 text-xs hover:bg-muted">{'\u4e0a\u4e00\u5468'}</button>
            <button type="button" onClick={() => setScheduleWeekStart(currentWeekStart)} className="rounded-md border border-border px-3 py-1.5 text-xs hover:bg-muted">{'本周'}</button>
            <button
              type="button"
              onClick={() => {
                const copiedIds = copyPreviousWeekSchedule()
                showToast(copiedIds.length > 0 ? `\u5df2\u590d\u7528\u4e0a\u5468 ${copiedIds.length} \u4e2a\u4efb\u52a1` : '\u4e0a\u5468\u6ca1\u6709\u53ef\u590d\u7528\u7684\u4efb\u52a1', copiedIds.length > 0 ? 'success' : 'info')
              }}
        ...
        <div className="border-b border-border px-4 py-2 text-xs text-muted-foreground md:hidden">{todayStatusText}</div>
        ...
        <div className="min-h-0 flex-1 overflow-auto custom-scrollbar p-3">
          <div className="min-w-[980px] overflow-hidden rounded-lg border border-border">
            <div className="grid grid-cols-[128px_repeat(7,minmax(120px,1fr))] bg-muted/60 text-xs font-medium">
              <div className="border-r border-border px-3 py-2">{'\u4efb\u52a1\u5217\u8868'}</div>
              {weekDates.map((date, index) => (
                <div key={formatDateKey(date)} className="border-r border-border px-3 py-2 last:border-r-0">
                  <div>{DAY_LABELS[index]}</div>
                  <div className="mt-0.5 text-[11px] font-normal text-muted-foreground">{formatDateKey(date).slice(5)}</div>
                </div>
              ))}
            </div>
            {sortedRows.map((row) => (
              <div key={row.id} className="grid min-h-[88px] grid-cols-[128px_repeat(7,minmax(120px,1fr))] border-t border-border">
                <ScheduleRowHeader
                  row={row}
                ...
                {weekDates.map((date) => {
                  ...
                  return (
                    <div
                      ...
                      className={`min-h-[88px] border-r border-border p-1.5 last:border-r-0 ${dragOverCell === cellKey ? 'bg-blue-500/10 ring-1 ring-inset ring-blue-500/40' : 'bg-background'}`}
                    >
          ...
          <button type="button" onClick={addScheduleRow} className="mt-2 inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs hover:bg-muted">
            <PlusIcon className="h-3.5 w-3.5" />
            {'\u6dfb\u52a0\u4efb\u52a1\u884c'}
          </button>
        </div>

        <div className="flex min-h-0 flex-1 flex-col border-t border-border bg-muted/20 p-3">
          <div className="mb-2 flex items-center justify-between gap-2">
            <div className="text-sm font-semibold">{'\u6536\u85cf\u4efb\u52a1\u5361'}</div>
            <select value={selectedCollectionId} onChange={(event) => setSelectedCollectionId(event.target.value)} className="h-8 rounded-md border border-border bg-background px-2 text-xs outline-none">
          ...
          <div className="grid min-h-0 flex-1 auto-rows-min grid-cols-1 content-start items-start gap-2 overflow-y-auto custom-scrollbar pr-1 sm:grid-cols-2 lg:grid-cols-3">
              ...
              return (
                <div
                  ...
                  onDragStart={(event) => {
                    ...
                    event.dataTransfer.setData(DRAG_TYPE, JSON.stringify({ taskId: task.id, collectionId: sourceCollectionId }))
                  }}
                  onDragEnd={() => setDragOverCell(null)}
                  onDoubleClick={() => openTaskDetail(task.id)}
                  className="grid min-h-[72px] cursor-pointer grid-cols-[auto_minmax(0,1fr)] gap-2 rounded-lg border border-border bg-background p-2 text-xs shadow-sm"
                >
                  <ScheduleTaskPreview task={task} />
                  <div className="grid min-w-0 grid-rows-[auto_auto_auto] gap-1.5">
                    <div className="flex min-w-0 items-center justify-between gap-2">
                      <div className="min-w-0 truncate font-medium" title={task.prompt}>{getTaskTitle(task)}</div>
                      <span className="shrink-0 rounded bg-yellow-500/10 px-1.5 py-0.5 text-[10px] text-yellow-600 dark:text-yellow-300">{task.params.n} {'\u5f20'}</span>
                    </div>
                    <div className="flex min-w-0 items-center justify-between gap-2 text-[11px] text-muted-foreground">
                      <div className="min-w-0 truncate">{getCollectionName(sourceCollectionId)}</div>
                      <label className="flex shrink-0 items-center gap-1.5" onDoubleClick={(event) => event.stopPropagation()}>
                        <input
                          type="checkbox"
                    ...
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="min-w-0 flex-1 rounded-md border border-border bg-background px-2 py-1 text-[11px] outline-none focus:ring-1 focus:ring-blue-500/40"
                      />
                      <button type="button" onClick={() => void handlePickOutputPath(task)} onDoubleClick={(event) => event.stopPropagation()} className="rounded-md border border-border p-1.5 hover:bg-muted" title="\u9009\u62e9\u6587\u4ef6\u5939">
            ...
            {filteredTasks.length === 0 && (
              <div className="col-span-full rounded-lg border border-dashed border-border p-4 text-center text-xs text-muted-foreground">{'\u6682\u65e0\u6536\u85cf\u4efb\u52a1'}</div>
            )}
          </div>
        </div>
      </div>
...
function ScheduleRowHeader({
  ...
  return (
    <div className="flex min-w-0 items-start gap-1 border-r border-border bg-muted/30 px-2 py-2 text-xs font-medium">
      <input
        value={draftName}
        ...
        className="min-w-0 flex-1 rounded border border-transparent bg-transparent px-1 py-0.5 outline-none hover:border-border focus:border-blue-500/40 focus:bg-background"
        title="任务行名称"
      />
      <button
        type="button"
        onClick={onRemove}
        disabled={!canRemove}
        className="shrink-0 rounded p-1 text-muted-foreground hover:bg-red-500/10 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-muted-foreground"
...
function ScheduleTaskPreview({ task }: { task: TaskRecord }) {
  ...
  return (
    <>
      <div
        className="h-[74px] w-[74px] shrink-0 overflow-hidden rounded-md border border-border bg-muted"
...
function ScheduleCellItem({
  ...
  return (
    <div
      ...
      className="cursor-move rounded-md border border-blue-500/30 bg-blue-500/10 p-1.5 text-[11px]"
    >
      <div className="mb-1 flex items-start justify-between gap-1">
        <div className="min-w-0">
          <div className="truncate font-medium" title={task?.prompt}>{task ? getTaskTitle(task) : '\u4efb\u52a1\u4e0d\u5b58\u5728'}</div>
          <div className="truncate text-[10px] text-muted-foreground">{collectionName} - {getScheduleStatusLabel(item.status)}</div>
        </div>
        <div className="flex shrink-0 items-center gap-0.5">
          <button type="button" onClick={onRunNow} onDoubleClick={(event) => event.stopPropagation()} className="rounded px-1.5 py-0.5 text-[10px] text-blue-600 hover:bg-blue-500/10 dark:text-blue-300" title="立即运行">
            运行
          </button>
          <button type="button" onClick={onRemove} onDoubleClick={(event) => event.stopPropagation()} className="rounded p-0.5 text-muted-foreground hover:bg-red-500/10 hover:text-red-500" title="\u79fb\u9664">
            <TrashIcon className="h-3 w-3" />
          </button>
        </div>
      </div>
      {item.lastError && (
        <div className="mb-1 max-h-8 overflow-hidden rounded bg-red-500/10 px-1.5 py-1 text-[10px] text-red-600 dark:text-red-300" title={item.lastError}>
          {item.lastError}
        </div>
      )}
