  ...
  return (
    <main className="min-h-dvh bg-[hsl(var(--ds-color-canvas))] px-4 py-8 text-[hsl(var(--ds-color-text))] sm:px-8">
      <div className="mx-auto grid w-full min-w-0 max-w-6xl grid-cols-[minmax(0,1fr)] gap-10">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-[hsl(var(--ds-color-text-muted))]">
                覆盖当前项目全部 68 个正式 UI 模块，并为高频重复模式提供可直接复用的组件。
        ...
        <Section title="全局 SVG 图标库" description="业务界面统一从 design-system/icons 取图标，默认 20px、2px 线宽，装饰图标自动隐藏给读屏器。">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
            {iconShowcase.map(([name, label]) => (
              <div key={name} className="flex min-h-20 flex-col items-center justify-center gap-2 rounded-ds-lg border border-ds-border bg-[hsl(var(--ds-color-surface-raised))] text-[hsl(var(--ds-color-text-muted))]">
        ...
        <Section title="反馈与系统状态" description="加载、空、成功、警告和错误都具有明确语义和恢复路径。">
          <div className="grid gap-4">
            ...
            <Alert tone="danger" title="API 配置不可用" actions={<Button size="sm" variant="secondary">检查设置</Button>}>
              当前配置无法建立连接，请检查地址、密钥和模型名称。
            </Alert>
            <Progress label="正在导出图片" value={68} showValue />
            <div className="flex flex-wrap gap-4">
              <StatusIndicator tone="success">已完成</StatusIndicator>
              <StatusIndicator tone="info" pulse>生成中</StatusIndicator>
            ...
            {!toastVisible && <Button size="sm" variant="secondary" onClick={() => setToastVisible(true)}>再次显示通知</Button>}
          </div>
        </Section>
        ...
        <Section title="数据展示" description="列表、卡片、键值、表格、缩略图和渐进披露共享统一层级。">
          <div className="grid gap-5 lg:grid-cols-2">
            <Card>
              ...
              <CardFooter>
                <Button size="sm" variant="secondary" leadingIcon={<CopyIcon size={14} />}>复制参数</Button>
            ...
            <Table>
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell><StatusIndicator tone="success">完成</StatusIndicator></TableCell>
                  <TableCell>18.4 秒</TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>详情页场景图</TableCell>
                  <TableCell><StatusIndicator tone="info">生成中</StatusIndicator></TableCell>
                  <TableCell>—</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </div>
          <CodeBlock language="命名模板" className="mt-5">{'{{date}}/{{preset}}/{{index}}.png'}</CodeBlock>
        </Section>

        <Section title="浮层与模态模式" description="Menu 处理命令，Popover 处理轻量设置，Dialog 和 Drawer 处理需要焦点管理的任务。">
          <Toolbar label="浮层示例">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            ...
            <Menu label="任务操作">
              <MenuItem icon={<CopyIcon size={15} />} shortcut={<Kbd>⌘D</Kbd>}>复制任务</MenuItem>
              <MenuItem icon={<FolderOpenIcon size={15} />}>打开输出目录</MenuItem>
              <MenuSeparator />
              <MenuItem icon={<TrashIcon size={15} />} tone="danger">删除任务</MenuItem>
            </Menu>
          </div>
          <div className="mt-5 overflow-hidden rounded-ds-xl border border-ds-border">
            <DialogWorkspace layout="triple" className="min-h-64">
              ...
              <DialogPane tone="content">
                ...
                <TextArea label="正文" className="mt-3 min-h-24" defaultValue="保持同一弹窗内的容器、字体、颜色和选择状态一致。" />
              </DialogPane>
            </DialogWorkspace>
          </div>
        </Section>
            ...
            <TableBody>
              {legacyComponentCoverage.map((entry) => (
                <TableRow key={entry.module}>
                  <TableCell><code className="text-xs">{entry.module.replace('src/', '')}</code></TableCell>
                  <TableCell>{entry.responsibility}</TableCell>
                  <TableCell>
                    <Badge tone={entry.decision === 'migrate' ? 'success' : entry.decision === 'compose' ? 'info' : 'neutral'}>
                      {entry.decision}
                    </Badge>
                  </TableCell>
            ...
            <TableBody>
              {interactionPatterns.map((pattern) => (
                <TableRow key={pattern.id}>
                  <TableCell><strong>{pattern.name}</strong></TableCell>
                  ...
                  <TableCell className="text-xs text-[hsl(var(--ds-color-text-muted))]">{pattern.relatedMaster}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
            ...
            <TableBody>
              {pageCoverage.map((page) => (
                <TableRow key={page.id}>
                  <TableCell><strong>{page.workspace}</strong></TableCell>
                  <TableCell><code className="text-xs">{page.entry}</code></TableCell>
                  <TableCell className="text-xs">{page.differences.join(' / ')}</TableCell>
                  <TableCell><code className="text-xs text-[hsl(var(--ds-color-text-muted))]">{page.document}</code></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <Dialog
        ...
        footer={
          <>
            ...
            <Button variant="danger" onClick={() => setDialogOpen(false)}>确认删除</Button>
          </>
        }
      >
