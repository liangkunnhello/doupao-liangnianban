...
export function getScheduleCompletionAction(item: ScheduleItem, relatedTasks: ScheduleRelatedTask[]): ScheduleCompletionAction {
...
export function getDueScheduleItemIds(items: ScheduleItem[], rows: ScheduleRow[], now = new Date()): string[] {
  ...
  const orderedToday = items
    ...
    .slice()
    .sort((a, b) => {
      const rowDiff = (rowOrder.get(a.rowId) ?? Number.MAX_SAFE_INTEGER) - (rowOrder.get(b.rowId) ?? Number.MAX_SAFE_INTEGER)
...
export function resolveScheduleOutputTarget({
  ...
  const fallbackCollectionId = collectionId && collections.some((collection) => collection.id === collectionId)
    ? collectionId
